...
import numpy as np
from a_estrella_heatmap import a_estrella
...
class Robot:
    ...
    idx_ruta: int = 0
    ticks_espera: int = 0
    ticks_espera_consecutivos: int = 0  # NUEVO: cuenta esperas seguidas
...
class SimAlmacen:
    def __init__(
        ...
        self.grid = grid
        # Actualizacion heatmap
        self.heatmap = np.zeros_like(self.grid, dtype=float)
        self.alpha_heatmap = 0.11  # Muy suave: penalizar sin bloquear rutas
    ...
    def _asignar_pedidos(self) -> None:
        ...
        for r in inactivos:
            ...
            # Considerar más pedidos para mejor distribución (200 en lugar de 50)
            for pi in self.pendientes[: min(1000, len(self.pendientes))]:
                p = self.pedidos[pi]
            ...
            #Modificacion heatmap
            ruta = a_estrella(self.grid, r.pos, pickup, self.heatmap, self.alpha_heatmap)
            if ruta is None:
    ...
    def _planear_siguiente_tramo_si_llego(self, r: Robot) -> None:
        ...
        if r.estado == "A_RECOGER":
            r.estado = "A_ESTACION"
            # Elegir la estación más cercana al robot en este momento (no la pre-asignada)
            mejor_dock = r.estacion_dock
            mejor_dist = 10**9
            for dock in self.estacion_dock.values():
                dist = abs(r.pos[0] - dock[0]) + abs(r.pos[1] - dock[1])
                if dist < mejor_dist:
                    mejor_dist = dist
                    mejor_dock = dock
            r.estacion_dock = mejor_dock
            # Modificacion heatmap
            ruta = a_estrella(self.grid, r.pos, r.estacion_dock, self.heatmap, self.alpha_heatmap)
            if ruta is None:
                # reintentar después
                r.estado = "A_RECOGER"
                return
    ...
    def step(self) -> None:
        # Limpiar reservas antiguas: ventana de 100 ticks
        self.tabla_reservas.actualizar_tick(self.tick)
        ...
        for r in self.lista_robots:
            ...
            if siguiente == actual:
                r.ticks_espera_consecutivos = 0  # No espera
            ...
            if self.tabla_reservas.puede_moverse(actual, siguiente, tick_siguiente):
                ...
                r.pos = siguiente
                # Actualizacion heatmap con incremento suave
                self.heatmap[siguiente[1], siguiente[0]] += 0.1
                r.idx_ruta += 1
                r.celdas_movidas += 1
                r.ticks_espera_consecutivos = 0  # Movió, reset contador
                movio_alguien = True
            else:
                r.ticks_espera += 1
                r.ticks_espera_consecutivos += 1
                self.eventos_alto += 1
                
                # REPLANIFICACIÓN: si espera > 5 ticks consecutivos, recalcular ruta
                if r.ticks_espera_consecutivos > 5 and r.estado in ["A_RECOGER", "A_ESTACION", "RETORNO"]:
                    objetivo = None
                    if r.estado == "A_RECOGER" and r.anaquel_home:
                        objetivo = elegir_objetivo_adyacente(self.grid, r.anaquel_home)
                    elif r.estado == "A_ESTACION" and r.estacion_dock:
                        objetivo = r.estacion_dock
                    elif r.estado == "RETORNO" and r.anaquel_home:
                        objetivo = elegir_objetivo_adyacente(self.grid, r.anaquel_home)
                    
                    if objetivo and objetivo != r.pos:
                        nueva_ruta = a_estrella(self.grid, r.pos, objetivo, self.heatmap, self.alpha_heatmap)
                        if nueva_ruta and len(nueva_ruta) > 1:
                            r.ruta = nueva_ruta
                            r.idx_ruta = 0
                            r.ticks_espera_consecutivos = 0  # Reset por replanificación
                
                self.tabla_reservas.confirmar_espera(r.robot_id, actual, tick_siguiente)

        # Decaimiento del heatmap: reduce valores anteriores para adaptarse a condiciones actuales
        self.heatmap *= 0.95
