...
def a_estrella(grid: np.ndarray, inicio: Celda, meta: Celda, heatmap: Optional[np.ndarray]=None, alpha: float=0.0) -> Optional[List[Celda]]:
    ...
    # Normalizar heatmap una vez para que la penalización esté acotada en [0, alpha]
    heatmap_max = float(heatmap.max()) if heatmap is not None else 0.0
    heatmap_norm = heatmap / heatmap_max if (heatmap is not None and heatmap_max > 0) else None
    ...
    while abiertos:
        ...
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
            ...
            # Modificacion heatmap: penalización basada en el valor NORMALIZADO del heatmap
            penalizacion = 0.0
            if heatmap_norm is not None:
                penalizacion = alpha * float(heatmap_norm[ny, nx])
