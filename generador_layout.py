...
def generar_layout(seed: int, ancho: int, alto: int, estaciones: int) -> Dict:
    """
    Concepto del layout:
    - Frente sur: mitad de estaciones centradas (y=H-2) con buffer (y=H-3)
    - Frente norte: mitad de estaciones centradas (y=1) con buffer (y=2)
    ...
    n_inferior = estaciones // 2
    n_superior = estaciones - n_inferior

    # --- Estaciones en el SUR (inferior) ---
    y_estacion_inf = alto - 2
    y_buffer_inf = alto - 3

    ancho_total_inf = n_inferior * 2
    x_inicio_inf = (ancho - ancho_total_inf) // 2

    # Carril buffer inferior como LIBRE
    for x in range(1, ancho - 1):
        if grid[y_buffer_inf, x] != BLOQUEADO:
            grid[y_buffer_inf, x] = LIBRE

    for i in range(n_inferior):
        sx = x_inicio_inf + i * 2
        for dx in (0, 1):
            if _en_rango(grid, sx + dx, y_estacion_inf):
                grid[y_estacion_inf, sx + dx] = ESTACION
        dock = (sx, y_buffer_inf)
        cell = (sx, y_estacion_inf)
        lista_estaciones.append(Estacion(estacion_id=i, dock=dock, cell=cell))

    # Apron inferior
    _recortar_rectangulo(grid, 1, y_buffer_inf - 2, ancho - 2, 2, LIBRE)

    # --- Estaciones en el NORTE (superior) ---
    y_estacion_sup = 1
    y_buffer_sup = 2

    ancho_total_sup = n_superior * 2
    x_inicio_sup = (ancho - ancho_total_sup) // 2

    # Carril buffer superior como LIBRE
    for x in range(1, ancho - 1):
        if grid[y_buffer_sup, x] != BLOQUEADO:
            grid[y_buffer_sup, x] = LIBRE

    for i in range(n_superior):
        sx = x_inicio_sup + i * 2
        for dx in (0, 1):
            if _en_rango(grid, sx + dx, y_estacion_sup):
                grid[y_estacion_sup, sx + dx] = ESTACION
        dock = (sx, y_buffer_sup)
        cell = (sx, y_estacion_sup)
        lista_estaciones.append(Estacion(estacion_id=n_inferior + i, dock=dock, cell=cell))

    # Apron superior
    _recortar_rectangulo(grid, 1, y_buffer_sup + 1, ancho - 2, 2, LIBRE)

    # Zona de parking/carga
    ancho_parking, alto_parking = 21, 16
    ...
    # Región de almacenaje (entre apron superior e inferior)
    y_top = y_buffer_sup + 3
    y_bottom = y_buffer_inf - 4
    x_left = 2
    x_right = ancho - 3
    ...
    for est in lista_estaciones:
        ...
        if not alcanzable[dy, dx]:
            # Estaciones inferiores: abrir hacia arriba; superiores: abrir hacia abajo
            if dy < alto // 2:
                _recortar_rectangulo(grid, dx, dy, 2, 6, LIBRE)
            else:
